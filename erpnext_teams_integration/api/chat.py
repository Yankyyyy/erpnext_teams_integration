...
def create_group_chat_for_doc(docname, doctype):
    ...
    if my_azure:
        members.append({'@odata.type':'#microsoft.graph.aadUserConversationMember','roles':['owner'],'user@odata.bind':f"https://graph.microsoft.com/v1.0/users('{my_azure}')"})
    if doctype == 'Event' and getattr(doc, 'event_participants', None):
        for p in doc.event_participants:
            ...
            if azure:
                members.append({'@odata.type':'#microsoft.graph.aadUserConversationMember','roles':['owner'],'user@odata.bind':f"https://graph.microsoft.com/v1.0/users('{azure}')"})
    if not members:
        frappe.throw('No valid Microsoft users to create chat.')
    payload = {'chatType':'group','topic': f"{doctype} {docname}", 'members': members}
    ...
    if res.status_code in (200,201):
        ...
        conv.insert(ignore_permissions=True); frappe.db.commit()
...
def _save_message_local(msg_json, chat_id, docname=None, doctype=None, direction='inbound'):
    ...
    created = msg_json.get('createdDateTime') or now_datetime()
    sender = msg_json.get('from', {}).get('user') or msg_json.get('from')
    sender_id = None; sender_display = None
    ...
    doc = frappe.get_doc({'doctype':'Teams Chat Message','chat_id':chat_id,'message_id':mid,'sender_id':sender_id,'sender_display':sender_display,'body':body,'created_at':created,'event': (docname if doctype=='Event' else None),'direction':direction})
